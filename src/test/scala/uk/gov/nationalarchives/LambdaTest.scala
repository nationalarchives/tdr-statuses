...
class LambdaTest extends TestUtils with BeforeAndAfterAll {
  ...
  val ffidResults: TableFor4[String, List[String], String, String] = Table(
    ...
    ("standard", List(s"$activeDisallowedStandardPuid", s"$inactiveDisallowedStandardPuid"), "1", "MultipleFormats"),
    ("judgment", List(s"$allowedJudgmentPuid", s"$allowedStandardPuid"), "1", "MultipleFormats")
  )
